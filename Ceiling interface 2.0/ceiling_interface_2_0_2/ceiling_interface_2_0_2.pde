...
void mouseReleased(){
  ...
  for (int i = 0; i < flood_lights.length; i++) {
    ...
    if (result == 0 && flood_lights[i].combine_check){ // combine variable was toggled and and new status is 'true'  
  ...
  // RGB SPOTS
    ...
    if (result == 0 && rgb_spots[i].combine_check){ // combine variable was toggled and and new status is 'true'  
    ...
    if (result == 1){ // a gui was opened
      if (open_gui_rgb_spots != -1) rgb_spots[open_gui_rgb_spots].gui_enabled = false; // if a gui was already open, close it
