/* ToDo
  Reimplement so that single select is possible, and single selections doesnt make lamp enherit the combined values, overwriting it's own original values
...
void setup(){
  ...
  dbg = new DimmerButtonGrid(35+20, 55); // setup dimmer button grid
  ...
  all_exhibition_rgb = new EquipmentButton(40+137/2, 599+185+40, "exhibition groupe");
  all_exhibition_rgb.setType("flood");
  all_exhibition_rgb.setSize(137, 40);
  all_exhibition_rgb.setFont(font_ArialMT_15);
  
  all_floor_rgb = new EquipmentButton(40+137/2, 599+185+40+50, "floor RGB groupe");
  all_floor_rgb.setType("flood");
  all_floor_rgb.setSize(137, 40);
  all_floor_rgb.setFont(font_ArialMT_15);
  
  all_floor_flood = new EquipmentButton(40+137/2+137+10, 599+185+40, "floor Flood groupe");
  all_floor_flood.setType("flood");
  all_floor_flood.setSize(137, 40);
  all_floor_flood.setFont(font_ArialMT_15);
  
  section_combined_rgb_spots = new SliderSection(35+20, 344+40);
  ...
  section_combined_floods = new SliderSection(35+20, 599+40);
...
void draw(){
  ...
  drawFrame(20+20, 344, 284, 235, label); // 325 , 549
  label = "Grouped lights : Flood";
  drawFrame(20+20, 599, 284, 185, label); // 804
  drawRig(360+20, 70);
...
void mouseReleased(){
  ...
  updateCombinedGUI();
  dbg.update(mouseX, mouseY, mouseButton);
}
...
void buttonManager(int mouse_x, int mouse_y, int mouse_button){
  ...
  for (int i = 0; i < eba.length; i++){ // traverse all the buttons in Equipment Button Array (eba)
    ...
    if (result == ENABLED_STATE){ // the combine state of this button was changed
      
      if (eba[i].enabled_state){ // this button was added to combine
  ...
  if (result == 0){
    if (trigger_button.enabled_state) {
      ...
      for (int i = 0; i < eb_array.length; i++) {
        ...
        eb_array[i].enabled_state = false;
      }
    }
  }
}
